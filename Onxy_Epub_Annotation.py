...
os.mkdir(f'E:\OneDrive\Books\Book Annotations\{annotation_name}')

final_annotation = open(f'E:/OneDrive/Books/Book Annotations/'
                        f'{annotation_name}/{annotation_name}.txt', 'a', encoding='UTF-8')

# Identify each type (highlight marker, highlight, annotation, and separator) and convert into markdown format.

i = 0
while i < len(lines):
    # Remove the annotation head with marker '读书笔记'.
    if lines[i] == '\n' or '读书笔记' in lines[i]:
        i += 1
        continue
    ...
    # Remove the empty lines until hitting the next separator.
        ...
        temp_line = '<font color=\'' + color_selection + '\'>' + temp_line + '</font>' + '\n\n'
        j = i + 1
        while j < len(lines):
            if lines[j] == '\n':
                j += 1
